      ...
      <CardContent className="space-y-3">
        <div className="relative z-20 space-y-2">
          ...
          <div className="relative">
            ...
            {query.trim().length >= 3 ? <div className="kita-scrollbar absolute inset-x-0 top-full mt-1 max-h-64 overflow-y-auto rounded-xl border border-border bg-popover text-popover-foreground shadow-xl">
        ...
        {routeProperties ? <div className="space-y-2 rounded-xl border border-border p-3">
          ...
          {instructions.length ? <ol className="kita-scrollbar max-h-52 space-y-2 overflow-y-auto border-t border-border pt-2 text-sm sm:max-h-64">{instructions.map((step, index) => <li key={`${index}-${step.name ?? "step"}`} className="flex gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-muted text-[11px]">{index + 1}</span><span className="min-w-0">{step.instruction?.text || step.name}{Number.isFinite(step.distance) && Number(step.distance) > 0 ? <span className="ml-1 text-xs text-muted-foreground">· {formatDistance(Number(step.distance))}</span> : null}</span></li>)}</ol> : <p className="border-t border-border pt-2 text-xs text-muted-foreground">Petunjuk belokan tidak tersedia untuk rute ini.</p>}
        </div> : null}
